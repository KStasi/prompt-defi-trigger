...
async function main() {
  ...
  const SimpleTriggerFactory = await hre.ethers.getContractFactory("SimpleTriggerFactory");
  ...
  const triggerConfigs = [
    ...
    // add more triggerConfigs if needed
  ];
  ...
  for (const config of triggerConfigs) {
    ...
    const triggerAddress = await simpleTriggerFactory.triggers(triggerCount - 1n);
